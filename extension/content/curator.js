// LinkedIn Jobs Curator — content script (ported from linkedin-job-filter.js)
//
// This is the userscript core, adapted for a Firefox WebExtension content script:
//   1. The // ==UserScript== metadata block is gone (see extension/manifest.json).
//   2. The IIFE is async and awaits window.__ljfStore.hydrate() so the synchronous
//      GM_getValue reads below resolve against a pre-populated cache (see content/store.js).
//   3. The history.pushState monkeypatch is replaced with a location poll — a content
//      script cannot patch what the page's own code sees.
// Everything else is unchanged from the userscript.

(async function () {
  'use strict';

  await window.__ljfStore.hydrate();
...
`;
  ...
  function renderJobsPane() {
    ...
    footerAddBtn.title = isDismView ? 'Add dismissed entry' : 'Add job manually (blank)';
    footerAddBtn.textContent = isDismView ? '+' : '_';
    footerRight.appendChild(toggleDiv); footerRight.appendChild(footerAddBtn);
    if (!isDismView) {
      const footerAutofillBtn = mk('button', 'ljf-footer-autofill', 'ljf-footer-add');
      footerAutofillBtn.title = 'Autofill from the job listing currently open';
      footerAutofillBtn.textContent = '+';
      footerRight.appendChild(footerAutofillBtn);
    }
    ...
    // Autofill from whatever job listing is currently open — populates the form but still
    // requires Save, so LinkedIn breaking the apply-flow selectors can't silently log a job.
    pane.querySelector('#ljf-footer-autofill')?.addEventListener('click', () => {
      const listing = detectCurrentJobListing();
      // Always reveal the form, same as the blank-add (_) button — whether or not detection
      // found anything, so a broken selector never looks like the button did nothing.
      editingLogIdx = null;
      editingDismissLogIdx = null;
      const formRow = pane.querySelector('#ljf-add-job-row');
      formRow.classList.add('ljf-open');
      pane.querySelector('#ljf-aj-company').value = listing ? listing.company : '';
      pane.querySelector('#ljf-aj-title').value   = listing ? listing.title   : '';
      pane.querySelector('#ljf-aj-date').value    = today;
      pane.querySelector('#ljf-aj-status').value  = 'applied';
      pane.querySelector('#ljf-aj-url').value     = listing ? listing.url : '';
      pane.querySelectorAll('tbody tr[data-log-idx]').forEach(r => r.classList.remove('ljf-row-editing'));
      pane.querySelector('#ljf-aj-company')?.focus();
      setStatus(listing
        ? '✓ Autofilled from current listing — review and Save.'
        : '⚠ No job listing detected on this page — form opened blank.');
    });
  ...
  // All applied-log entries whose company matches; when title is given (not undefined), further
  // restricted to entries whose (normalized) title matches it exactly. Status is never a factor —
  // a rejected or ghosted application is still an application.
  //
  // Exact, not substring: "Senior Product Manager" and "Senior Product Manager (Containers)" are
  // different postings at the same company, not the same job with extra decoration — a substring
  // match would treat every other role sharing that prefix as "already applied to this one".
  // Same-company-different-title falls through to matchJobLogCompanyOnly's "last applied" badge.
  function findLogEntries(company, title) {
    return appliedLog.filter(e =>
      logCompanyMatches(company, e.company) &&
      (title === undefined || (e.title && title === normalizeSenior(e.title).toLowerCase()))
    );
  }

  // The entry with the latest date in a list (undated entries sort last), or null if empty.
  // This is what "Applied on <date>" should always reflect — most recent, any status.
  function mostRecentEntry(entries) {
    if (!entries.length) return null;
    return entries.reduce((latest, e) => (!latest || (e.date || '') > (latest.date || '')) ? e : latest);
  }

  // Returns the most recently dated matching log entry if card's company+title match, else null.
  function matchJobLog(card) {
    ...
    return mostRecentEntry(findLogEntries(company, title));
  }
  ...
  function matchJobLogCompanyOnly(card) {
    ...
    const companyEntries = findLogEntries(company);
    if (companyEntries.length === 0) return null;
    const title = normalizeSenior(cardText(card, TITLE_SEL)).toLowerCase();
    if (findLogEntries(company, title).length > 0) return null; // exact match — handled by matchJobLog
    return mostRecentEntry(companyEntries).date || '';
  }
  ...
  function applyViewHeroRules() {
    ...
    // Job log — exact match (most recent dated entry, any status)
    const normTitle = normalizeSenior(title).toLowerCase();
    const logEntry = jobLogEnabled ? mostRecentEntry(findLogEntries(company.toLowerCase(), normTitle)) : null;

    // Job log — company-only match
    const logCompanyDate = (!logEntry && jobLogEnabled) ? (() => {
      const entries = findLogEntries(company.toLowerCase());
      return entries.length ? (mostRecentEntry(entries).date || '') : null;
    })() : null;
...
<div style="overflow-y:auto;padding:16px 20px;display:flex;flex-direction:column;gap:14px;">
  ...
  <div>
    ...
    <div style="${s(12, th.panelText, 'line-height:1.5;')}">In the Jobs tab, the <strong>+</strong> button autofills the add-job form from whatever listing you have open — review and hit Save. Track application status, see every role you've applied to at a given company, and review stats in the footer.</div>
  </div>
  ...
  function openSettingsModal() {
    ...
    function renderContent() {
      ...
      if (activeTab === 'Colors') {
        ...
        sheetsLabel.textContent = 'Google Sheets Sync';
        sheetsLabel.style.cssText = [
          `color:${th.countText}`, 'font-size:10px', 'font-weight:600',
          'letter-spacing:.5px', 'text-transform:uppercase',
          `border-top:1px solid ${th.border2}`, 'padding-top:8px', 'margin-top:2px',
        ].join(';');
        grid.appendChild(sheetsLabel);
        grid.appendChild(buildSheetsSyncSection());

        content.appendChild(grid);
      }
    }

    // Backup tab's Sheets-sync controls. Buttons here deliberately don't close the modal
    // (unlike mkBackupBtn) so the status line can update in place. All the real work \u2014 OAuth,
    // reading/writing the spreadsheet \u2014 happens in the background page; this just messages it.
    function buildSheetsSyncSection() {
      const wrap = document.createElement('div');
      wrap.style.cssText = 'display:flex;flex-direction:column;gap:6px;';

      const sheetInput = document.createElement('input');
      sheetInput.type = 'text';
      sheetInput.placeholder = 'Spreadsheet URL or ID';
      sheetInput.style.cssText = [
        `background:${th.inputBg}`, `color:${th.inputText}`,
        `border:1px solid ${th.inputBorder}`, 'border-radius:4px',
        'padding:6px 8px', 'font-size:12px', 'width:100%', 'box-sizing:border-box',
      ].join(';');
      wrap.appendChild(sheetInput);

      function mkPlainBtn(label) {
        const btn = document.createElement('button');
        btn.textContent = label;
        btn.style.cssText = [
          `background:${th.rowBg}`, `color:${th.panelText}`,
          `border:1px solid ${th.rowBorder}`, 'border-radius:4px',
          'padding:8px 10px', 'cursor:pointer', 'font-size:12px', 'flex:1',
        ].join(';');
        return btn;
      }
      const btnRow = document.createElement('div');
      btnRow.style.cssText = 'display:flex;gap:6px;';
      const authBtn = mkPlainBtn('Authorize');
      const syncNowBtn = mkPlainBtn('Sync now');
      syncNowBtn.hidden = true;
      btnRow.appendChild(authBtn);
      btnRow.appendChild(syncNowBtn);
      wrap.appendChild(btnRow);

      const statusLine = document.createElement('div');
      statusLine.style.cssText = `font-size:11px;color:${th.countText};padding:2px 0;`;
      ...
      function render(status) {
        status = status || {};
        if (status.spreadsheetId && document.activeElement !== sheetInput) {
          sheetInput.value = status.spreadsheetId;
        }
        isConnected = Boolean(status.connected);
        authBtn.textContent = isConnected ? 'Disconnect' : 'Authorize';
        syncNowBtn.hidden = !isConnected;
        let msg = connected
          ? 'Connected' + (status.lastSyncAt ? ' \u2014 last synced ' + new Date(status.lastSyncAt).toLocaleString() : '')
          : 'Not connected';
        if (status.lastError) msg += ' \u2014 error: ' + status.lastError;
        statusLine.textContent = msg;
      }

      function refresh() {
        browser.runtime.sendMessage({ type: 'ljf:get-status' })
          .then(render)
          .catch(() => { statusLine.textContent = 'Sync unavailable (background not responding).'; });
      }

      authBtn.addEventListener('click', () => {
        if (isConnected) {
          authBtn.disabled = true;
          browser.runtime.sendMessage({ type: 'ljf:disconnect' })
            .then(render)
            .finally(() => { authBtn.disabled = false; });
          return;
        }
        const spreadsheetId = extractSpreadsheetId(sheetInput.value);
        if (!spreadsheetId) { statusLine.textContent = 'Enter a spreadsheet URL or ID first.'; return; }
        authBtn.disabled = true;
        authBtn.textContent = 'Authorizing\u2026';
        browser.runtime.sendMessage({ type: 'ljf:connect', spreadsheetId })
          .then(render)
          .finally(() => { authBtn.disabled = false; });
      });
      ...
      syncNowBtn.addEventListener('click', () => {
        ...
        browser.runtime.sendMessage({ type: 'ljf:sync-now' })
          .then(render)
          .finally(() => { syncNowBtn.disabled = false; syncNowBtn.textContent = 'Sync now'; });
      });
...
<button class="ljf-del ljf-btn-del" data-id="${rule.id}" title="Delete rule">✕</button>`;
  ...
  //
  // LinkedIn's job-detail markup is now atomic-CSS: every class name is a short hash that's
  // regenerated per deploy, so none of them are usable as selectors (confirmed against a
  // fresh page capture — the old .jobs-unified-top-card__* classes no longer appear at all).
  // What *is* stable: the URL. The job ID comes from the address bar, the title link is the
  // one anchor whose href contains that ID (always unique on the page), and the company link
  // is whichever company-profile link sits nearest that title anchor in the DOM. document.title
  // ("<Title> | <Company> | LinkedIn") backs up whatever that DOM pass doesn't find — it's what
  // already covered standalone /jobs/view/ pages before this rewrite.

  // Walks up from a title anchor to the nearest ancestor containing a link to a company
  // profile. Scoping the search this way — instead of a page-wide first match — keeps it from
  // picking up an unrelated company link from a "similar jobs" or sidebar section.
  // Skips matches with no visible text: LinkedIn's markup nests a second, inner <a> (the
  // visible company name) inside the outer company-link wrapper, which HTML doesn't actually
  // allow — browsers auto-close the outer <a> where the inner one starts, leaving it a
  // separate, text-less anchor to the same URL that would otherwise match first.
  function nearestCompanyLink(fromEl, maxHops = 10) {
    let node = fromEl;
    for (let i = 0; i < maxHops && node.parentElement; i++) {
      node = node.parentElement;
      for (const link of node.querySelectorAll('a[href*="linkedin.com/company/"]')) {
  ...
  // Reads title/company/URL for whatever job listing is currently on screen — the search-results
  // detail pane, or a standalone /jobs/view/ page. Returns null if nothing is detected. Backs the
  // Jobs tab's "autofill from current listing" button.
  function detectCurrentJobListing() {
    const jobId = (window.location.pathname.match(/\/jobs\/view\/(\d+)/) || [])[1]
      || new URLSearchParams(window.location.search).get('currentJobId');

    let title = '', company = '';
    if (jobId) {
      const titleEl   = document.querySelector('a[href*="/jobs/view/' + jobId + '"]');
      const companyEl = titleEl ? nearestCompanyLink(titleEl) : null;
      title   = titleEl   ? titleEl.textContent.trim()   : '';
      company = companyEl ? companyEl.textContent.trim() : '';
      dlog('DOM parse:', { jobId, title, company, titleElFound: !!titleEl, companyElFound: !!companyEl });
    }
    ...
    if (!title || !company) {
      ...
      dlog('document.title parse:', { title, company, docTitle: document.title });
    }
  ...
  // ─── Dismiss / undo capture ──────────────────────────────────────────────────

  function setupDismissCapture() {
    document.addEventListener('click', e => {
      ...
      // Guard against modal close buttons (Easy Apply, network popup) that match
      // DISMISS_SEL but should not trigger a dismissal log entry.
      const dismissBtn = e.target.closest(DISMISS_SEL);
      if (dismissBtn && !dismissBtn.closest('[role="dialog"]')) {
  ...
  function init() {
    ...
    // Another LinkedIn tab (or a future sync layer) changed storage — reload the arrays
    // from the (already-refreshed) cache and re-scan. Skipped while the user is mid-edit.
    window.__ljfOnExternalChange = (keys) => {
      try {
        if (editingRuleId != null || editingLogIdx != null || editingDismissLogIdx != null) return;
        if (!keys || keys.includes('ljf_rules'))       { rules = loadRules(); }
        if (!keys || keys.includes('ljf_applied_log')) { appliedLog = loadAppliedLog(); logIndex = buildLogIndex(); }
        if (!keys || keys.includes('ljf_dismiss_log')) { dismissLog = loadDismissLog(); dismissLogIndex = buildDismissLogIndex(); }
        buildPanelContent();
        applyAllRules();
      } catch (e) { console.warn('[LJF] external-change refresh failed', e); }
    };
